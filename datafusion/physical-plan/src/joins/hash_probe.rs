...
impl ExecutionPlan for HashProbeExec {
    ...
    fn execute(
        ...
        let left_fut = match self.mode {
            ...
            PartitionMode::Auto => {
                return plan_err!(
                    "Invalid HashProbeExec, unsupported PartitionMode {:?} in execute()",
                    PartitionMode::Auto
                );
            }
        };
...
mod tests {
    ...
    async fn partitioned_join_collect(
        ...
        let join = HashProbeExec::try_new(
    ...
    async fn partitioned_join_overallocation() -> Result<()> {
        // Prepare partitioned inputs for HashProbeExec
